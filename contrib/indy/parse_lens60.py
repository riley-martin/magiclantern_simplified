...
while lens_id > 0:
  ...
  if lens_id > 0:
    ...
    for t in range(0x38c,0x38c+12*16,12):
      print(hexlify(val[t:t+12]), end=' ')
    print()
    print(hexlify(val[0x44c:0x44c+4]))
    print(hexlify(val[0x450:0x452]), hexlify(val[0x452:0x454]), hexlify(val[0x454:0x456]),hexlify(val[0x456:0x458]), end=' ')
